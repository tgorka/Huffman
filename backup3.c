...
struct TDrzewo {
    char slowo;
    int waga;
    //Do poruszania się po prefixach.
    TDrzewo *prefix;
    TDrzewo *postfix;
    //Do poruszania się od pnia.
    TDrzewo *lewy;
    TDrzewo *prawy;
    TDrzewo *korzen;
    //Do wzrostu drzewa.
...
int KSortuj(TDrzewo *Drzewo);           //sortuje elementy rosnąco względem wagi po *nast-ach - szykowanie do generowania drzewa.
...
TDrzewo* KZnajdz(TDrzewo *Prefix, char slowo);  //Znajduje słowo w drzewie i zwraca jego prefix.
TCiag* KZnajdzSkrot(TDrzewo *Prefix);   //Generuje skrót do danego prefixu z drzewa Huffmana;
char KDlogoscC(TCiag *Ciag);           //Generuje długość srótu w postaci znaku.

int KZapiszSlownik(TDrzewo *Prefix, char *nazwa);  //Zapisuje słownik do pliku.
TCiag* KompresujPlikDoCiaguB(TDrzewo *Prefix, char *nazwa);   //Generuje ciąg bitowy zakompresowanych danych.
TCiag* KCiagDanych(TCiag *CiagB);           //Generucje ciąg znaków do zapisu (KOMPRESJA).
int KZapiszPlk(TCiag *Ciag, char *nazwa);    //Zapisuje Ciąg do pliku.

int Kompresuj (char *nazwaOdcz, char *NazwaZap);        //Kompresja metodą Huffmana.
...
TDrzewo* KPobierzElementy(char *nazwa){
    ...
    while (fread(&znak,sizeof(znak),1,plk)!=0){
        i=0;
        while (i==0){
            //Jeśli slowo jest wśród elementów.
...
TCiag* KompresujPlikDoCiaguB(TDrzewo *Prefix, char *nazwa){
    ...
    while ((fread(&znak, sizeof(znak),1, plk))!=0){
        ...
        //Dopisywanie ciągu temp do Ciągu Ciąg.
        while (temp!=NULL){
            if (Ciag==NULL) {
                if ((Ciag=malloc(sizeof(TCiag)))==NULL) return NULL;
                BCiag=Ciag;
